...
<ul>
  ...
  <li>🔐 Session Management using Session ID</li>
  <li>🎟️ Session Token-based request tracking</li>
  <li>🛡️ CSRF Protection for secure request handling</li>
</ul>
...
<p>
This project includes basic security enhancements to simulate real-world backend protection mechanisms:
</p>

<ul>
  <li><strong>Session Management:</strong> Each user interaction is tracked using unique session IDs generated by the server.</li>
  
  <li><strong>Session Tokens:</strong> Token-based identification is used to maintain user state across multiple requests.</li>
  
  <li><strong>CSRF Protection:</strong> Implemented Cross-Site Request Forgery protection to prevent unauthorized actions from external sources.</li>
  
  <li><strong>Secure API Handling:</strong> Sensitive operations are validated before execution to ensure safe data transactions.</li>
</ul>

<p>
These features demonstrate an understanding of backend security principles and can be extended further using Spring Security and JWT authentication.
</p>
